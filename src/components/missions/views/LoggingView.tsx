        ...
        <section className={cn(PANEL_STYLE, "p-4 flex flex-col gap-2")}>
          ...
          {noteLog.length > 0 ? (
            ...
            <p className="mt-2 text-xs text-zinc-500">
              No session notes yet. Notes captured during planning, setup, and capturing
              will appear here.
            </p>
          )}
          <p className="mt-auto pt-3 text-[11px] text-zinc-500">
            Logging stage is read-only for the session log. Use the per-target cards and
            overall notes below to summarize results.
          </p>
        </section>
        ...
        >
          <div className="flex items-center justify-between mb-3 text-sm">
            ...
            <span className="text-[11px] text-zinc-500">
              Frozen snapshot from the session
            </span>
          </div>
          {mission?.targets?.length ? (
            <div className="flex-1 min-h-0 flex flex-col">
              <MissionTimeline
                ...
                hero
                selectedTargetId={mission.currentTargetId ?? null}
                currentTargetId={mission.currentTargetId ?? undefined}
                fieldMode={false}
              />
            </div>
          ) : (
            <p className="text-xs text-zinc-500">
              Timeline will appear here once targets are added to this mission.
            </p>
          )}
        </section>
      </div>
      ...
      <div className="grid grid-cols-1 gap-4 lg:grid-cols-[minmax(0,1.6fr)_minmax(0,1.4fr)]">
        ...
        <section className={cn(PANEL_STYLE, "p-4 space-y-4")}>
          <div className="flex items-center justify-between mb-1">
            ...
            <span className="text-[11px] text-zinc-500">
              Read-only · logged after capture
            </span>
          </div>
          ...
          {mission.targets.length === 0 ? (
            <div className="py-6 text-center rounded-lg border border-dashed border-white/10">
              <p className="text-sm text-zinc-500">
                No targets were configured for this mission.
              </p>
            </div>
          ) : (
            <div className="space-y-2">
              {mission.targets.map((t, idx) => {
                ...
                const logState = perTargetState.get(t.targetId);
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div className="flex-1 min-w-0">
                        ...
                        <p className="mt-0.5 text-[11px] text-zinc-500">
                          Planned recipe:{" "}
                          {t.subLength ?? logState?.subLengthSeconds ?? 120}
                          s · {t.frames ?? logState?.framesCaptured ?? 60} frames
                        </p>
                      </div>
                      <button
                        type="button"
                      ...
                      >
                        {expandedTargets.has(t.targetId) ? "Hide log" : "Show log"}
                      </button>
                    </div>

                    {expandedTargets.has(t.targetId) && (
                      <div className="pt-2 border-t border-white/5 space-y-2">
                        <div className="flex flex-wrap items-center gap-2">
                          ...
                          <div className="flex flex-wrap gap-1.5">
                            {(["success", "partial", "failed"] as TargetResultStatus[]).map(
                              (status) => {
                                const labels: Record<TargetResultStatus, string> = {
                                  success: "Success",
                                  partial: "Partial",
                                  failed: "Failed",
                                };
                        ...
                        <div className="space-y-1">
                          ...
                          <textarea
                            ...
                            placeholder="What worked, what didn’t, anything to remember for next time…"
        ...
        <section className={cn(PANEL_STYLE, "p-4 space-y-3")}>
          ...
          <p className="text-xs text-zinc-400">
            Stamps captured during capturing are shown here. Use them to understand how
            conditions evolved across the session.
          </p>
